...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        # rowzero = self.zero_loc[0]
        # colzero = self.zero_loc[1]
        rowzero = 0
        colzero = 0
        ...
        for i in range(4):
            if i == 0 and rowzero == 2:
                continue
            if i == 1 and colzero == 0:
                continue
            if i == 2 and rowzero == 0:
                continue
            if i == 3 and colzero == 2:
                continue

            nextstate = [row[:] for row in self.state]
            
            
            if i == 0:
                nextstate[rowzero][colzero] = nextstate[rowzero + 1][colzero]
                nextstate[rowzero + 1][colzero] = 0
            if i == 1:
                nextstate[rowzero][colzero] = nextstate[rowzero][colzero - 1]
                nextstate[rowzero][colzero - 1] = 0
            if i == 2:
                nextstate[rowzero][colzero] = nextstate[rowzero - 1][colzero]
                nextstate[rowzero - 1][colzero] = 0
            if i == 3:
            ...
            nbr_states.append(EightPuzzleState(nextstate, self.goal, self.dist_from_start + 1, use_heuristic= self.use_heuristic, zero_loc= [rowzero, colzero]))\
    ...
    def compute_heuristic(self):
        ...
        for i in range(len(self.state)):
            for j in range(len(self.state[i])):
                if(self.state[i][j] == 0):
                    continue
                actualx = (self.state[i][j]) // 3
                actualy = (self.state[i][j]) % 3
                total += manhattan((i,j),(actualx, actualy))
...
class SingleGoalGridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for a in neighboring_grid_locs:
            nbr_states.append(SingleGoalGridState(a, self.goal, self.dist_from_start + 1, self.use_heuristic, self.maze_neighbors))
        return nbr_states
...
class GridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        for a in neighboring_locs:
            if a in self.goal and 1 < len(self.goal):
                kneigoal = list(self.goal)
                kneigoal.remove(a)
                kneigoal= tuple(kneigoal)
                nbr_states.append(GridState(a, kneigoal, self.dist_from_start + 1, self.use_heuristic, self.maze_neighbors))
            else:
                nbr_states.append(GridState(a,self.goal, self.dist_from_start + 1, self.use_heuristic, self.maze_neighbors))
    ...
    def __eq__(self, other):
        return (self.state, self.goal) == (other.state, other.goal)
    ...
    def compute_heuristic(self):
        ...
        minman = 10000000000
        for a in self.goal:
            if(manhattan(self.state, a) < minman):
                minman = manhattan(self.state, a)
