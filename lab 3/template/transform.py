...
def transformToMaze(alien, goals, walls, window,granularity):
    ...
    start = configToIdx(alien.get_config(), [0,0,0], granularity, alien)
    
    toReturn = []
    for i in range(window[0]//granularity + 1):
        Row = []
        for j in range(window[1]//granularity + 1):
            entry = []
            for k in range(3):
                entry.append(SPACE_CHAR)
            Row.append(entry)
        toReturn.append(Row)
    ...
    for i in range(window[0]//granularity + 1):
        for j in range(window[1]//granularity + 1):
            ...
            for b in range(0,3):
                pos = idxToConfig((i, j, b), [0, 0, 0], granularity, alien)

                alien.set_alien_config(pos)

                if start[0] == i and start[1] == j and start[2] == b:
                    toReturn[i][j][b] = START_CHAR
    
                if does_alien_touch_wall(alien, walls, granularity) or is_alien_within_window(alien, window, granularity) == False:
                    toReturn[i][j][b] = WALL_CHAR
                    
                elif does_alien_touch_goal(alien, goals):
                    toReturn[i][j][b] = OBJECTIVE_CHAR

    return Maze(toReturn, alien, granularity=granularity)
